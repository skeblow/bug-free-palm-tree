...
export async function fetchAllBazar (): Promise<Array<Item>> {
  const urls = [
    'https://www.bazar.cz/subaru/?ro=1999&rd=2023&p2=500000&SP=14-71_38-100-999999999_',
  ...
  return elements.map((el: HTMLElement): Item => {
    ...
    const mileage = parseInt((el.querySelector('.najeto')?.textContent?.replace('Najeto', '')?.replace(/\s+/, '') ?? '0'))
